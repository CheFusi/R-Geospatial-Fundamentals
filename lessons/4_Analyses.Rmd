...
-   These relationships operations, like proximity and intersection,
    ...
    extracting data metrics and insightful subsets

In this lesson we explore the two basic types of spatial queries:
measurement queries and relationship queries.
...
#fix any potential issues with the counies geometry
...
-   When conducting measurement queries, it's essential to have
    consistent units to ensure accurate and meaningful analyses.
...
Subset data using and attribute query

Lets select data pertaining only to Alameda County and save it to a
spatial dataframe

\*\* Note that this is an attribute query which involves selecting or
filtering data based on specific attributes or properties within a
dataset
...
Let's try that conversion with the `uset_units`
...
⚠️ **Warning**: manual unit conversions using the `sf` package don't
translate well - converted values may still be reported as m2
...
They can differ greatly depending on the CRS. Let's take a look.
...
-   The `st_distance` function can be used to find the distance between
    two geometries or two sets of geometries.

Let's compute some distance queries.
...
#calculate the distance between Berkeley High and each portion of bike blv
...
There are different ways to determine which section of the bike
boulevard to calculate the distance to, for instance, to the nearest
point of the line geometry, which depend on the goal of the analysis.
See the `?st_distance()` documentation for more details.

Measurement queries focus on how the features of and **within** a single
geometry relate. Relationship queries relate elements of two geometries.
...
can be used to specify explicit relationshps 9(e.g. touches) by setting
the operation or `op=()` argument to any of the options listed in the
`?st_intersects` help documentation.
...
3.  On the same map, plot these schools in `"blue"`, the Los Angeles
    county boundary in `"purple"`, and the Alameda county boundary in
    `"red"`
...
### Relationship and Measurement Queries Combined - Proximity Analyses
...
common analysis that combines those concepts: promximity analysis.
...
One aspect of a walkable city is that services are accessible within a
certain walk time.

Let's consider the proximity of continuing education schools to BART
lines in Alameda county
...
#one, arbitrary, publication on walkablw cities: https://www.sciencedirect.com/science/article/pii/S2667091722000188
```
...
# highlight the schools interrsect the bike boulevart buffer in yellow
...
-   You can use `st_distance` and its companion function
    `st_nearest_feature` to compute the distance between each feature of
    A and the nearest in B e.g. each school and the nearest bike
    boulevard.

-   the `st_nearest_feature` function returns the ID of the closest
    feature.
...
#create new sf dataframes that only contains schools in the Berkeley Unified school district, 
...
the output: the `st_nearest_feature` function returns the ID of the
closest feature. These are stored in `nearest_elementary_middle_school`.

We can view the full information of the nearest middle school
...
elementary school, and the nearest middle school
